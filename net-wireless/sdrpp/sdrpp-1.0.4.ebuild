...
src_install() {
	...
	# fix bug '[warning] Module directory /usr/lib/sdrpp/plugins does not exist, not loading modules from directory'
	[[ -d "${D}/usr/$(get_libdir)/sdrpp/plugins" ]] && {
		mkdir -vp "${D}/usr/lib/sdrpp"
		ln -vs "${EPREFIX}/usr/$(get_libdir)/sdrpp/plugins" "${D}/usr/lib/sdrpp/plugins"
	}
}
